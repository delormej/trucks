...
namespace Trucks
{
    public class PantherClient
    {
        ...
        public async IAsyncEnumerable<KeyValuePair<string, SettlementHistory>> DownloadSettlementsAsync(DateTime settlementDate)
        {
            Func<SettlementHistory, bool> filter = (s => s.SettlementDate == settlementDate);
            await foreach (var s in DownloadSettlementsAsync(filter))
                yield return s;
        }
        ...
        /// </summary>
        public async IAsyncEnumerable<KeyValuePair<string, SettlementHistory>> DownloadSettlementsAsync(bool overwrite = false, int max = 10)
        {
        ...
        public async IAsyncEnumerable<KeyValuePair<string, SettlementHistory>> DownloadSettlementsAsync(Func<SettlementHistory, bool> filter, int max = 10)
        {
            ...
            await foreach (var s in DownloadSettlementsAsync(selectSettlements))
                yield return s;
        }
